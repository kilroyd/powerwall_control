#!/usr/bin/env python3
'''
Controls Tesla Powerwall by sending requests via the Developer API on
the Netzero app. Use of the Developer API on Netzero is currently
free.
...
def parse_args():
    parser = argparse.ArgumentParser(description='Tesla Powerwall Control via Netzero',
                                     epilog=__doc__,
                                     formatter_class=argparse.RawDescriptionHelpFormatter)

    parser.add_argument('--api-token', '-a', default=None,
                        help='API token to use')
    parser.add_argument('--system-id', '-s', default=None,
                        help='System to control')
    parser.add_argument('--system-json', '-j', default=None,
                        help='JSON file containing API token and System ID')

    parser.add_argument('--set-backup', '-b', type=int,
                        default=None,
                        help='Set backup reserve percentage')
    parser.add_argument('--set-mode', '-m', choices=['ss', 'tbc'],
                        default=None,
                        help='Set operational mode (self sufficiency or time based control)')
    parser.add_argument('--grid-charging', '-g',
    ...
    if args.system_json:
        with open(args.system_json, 'r', encoding='utf-8') as f:
            data = json.load(f)
    ...
    if not (args.api_token and args.system_id):
        print('error: API token and System ID must be specified, either by --system-json or --api-token and system-id')
        sys.exit(1)
...
def main():
    ...
    if args.set_mode:
        ...
        request['operational_mode'] = 'autonomous' if args.set_mode == 'tbc' else 'self_consumption'

    if args.grid_charging is not None:
        print(f' Grid charging allowed {args.grid_charging}')
        request['grid_charging'] = True if args.grid_charging else False
